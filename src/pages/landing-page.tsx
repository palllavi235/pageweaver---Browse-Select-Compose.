...
export function LandingPage() {
  return <div className="min-h-screen overflow-hidden bg-paper">
    <header className="page-shell flex h-20 items-center justify-between">
      ...
      <div className="flex items-center gap-2"><div className="hidden sm:block"><GoogleAuthButton compact /></div><Link to="/app"><Button variant="secondary">Open workspace <ArrowRight className="size-4" /></Button></Link></div>
    </header>
    <main>
      <section className="page-shell relative grid min-h-[760px] items-center gap-14 py-16 lg:grid-cols-[.9fr_1.1fr] lg:py-24">
        <div className="relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .24 }} className="mt-9 flex flex-col gap-3 sm:flex-row"><Link to="/app"><Button size="lg" className="w-full sm:w-auto">Start weaving <ArrowRight className="size-4" /></Button></Link><Button size="lg" variant="secondary"><Play className="size-4 fill-current" /> See how it works</Button></motion.div>
      ...
      <section id="how" className="border-y bg-surface py-24">
        <div className="page-shell"><div className="mx-auto max-w-2xl text-center"><p className="text-xs font-bold uppercase tracking-[.18em] text-leather">A better workflow</p><h2 className="mt-4 font-display text-4xl font-semibold tracking-tight sm:text-5xl">From scattered pages to finished work.</h2><p className="mt-5 text-muted">The familiar canvas experience, thoughtfully redesigned for documents.</p></div>
